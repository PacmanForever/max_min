...
- **Startup ordering (CRÍTIC)**: `start_listeners()` (watchdog + state listener) s'ha de cridar DESPRÉS de `async_forward_entry_setups()` (RestoreEntity). Si s'executa abans, el watchdog veu `last_reset=None` (sensor font unavailable al boot), dispara un reset fals, marca `_pending_start_reanchor`, i el primer state change sobreescriu start/end restaurats → delta=0. Seqüència obligatòria a `__init__.py`: (1) `first_refresh` → (2) `forward_entry_setups` → (3) `start_listeners()` → (4) `apply_pending_initials()`. Safety net: `update_restored_data()` neteja `_pending_start_reanchor` quan accepta start/end. Actualitzar docstrings de `coordinator.py` i `__init__.py` si l'ordre canvia.
