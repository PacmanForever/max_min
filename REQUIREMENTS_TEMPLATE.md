...
- **Persistence**: No persistent storage - max/min values are lost on Home Assistant restart
- **Restart Behavior**: Only current sensor value is preserved across HA restarts
...
- **Runtime Dependencies**: See `DEPENDENCIES.md` for detailed documentation
- **Testing Dependencies**: See `TESTING-DEPENDENCIES.md` for detailed documentation
...
- **Home Assistant Restart Scenarios**:
  - Max/min values are lost on HA restart (no persistence)
  - Current sensor value is preserved across restarts
  - Reset timing is recalculated after restart
  - Behavior during reset windows is maintained
  - Unavailable sensor state handling during restart
  - Multiple restart scenarios preserve current sensor readings

### Reliability
- **Home Assistant Restart Handling**: Component recovers gracefully from HA restarts
- **Data Loss**: Accumulated max/min values are not persisted (by design)
- **Reset Rescheduling**: Reset timers are recalculated after restart
- **Sensor State Preservation**: Current sensor readings are maintained across restarts
