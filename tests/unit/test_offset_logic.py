...
async def test_offset_dead_zone_ignore_updates(hass, config_entry):
    ...
    with patch("custom_components.max_min.coordinator.async_track_point_in_time"):
        ...
        with freeze_time("2023-01-01 23:59:40"):
            event = Mock()
            event.data = {"new_state": Mock(state="20.0", attributes={"state_class": "total_increasing"})}
        ...
        with freeze_time("2023-01-02 00:00:15"):
            event = Mock()
            event.data = {"new_state": Mock(state="50.0", attributes={"state_class": "total_increasing"})}
            coordinator._handle_sensor_change(event)
